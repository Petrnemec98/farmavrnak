...
script("$uf.onclick = function () { return confirm('".($Qd?js_escape($Qd):'Opravdu?')."'); };","");}function
...
cookie($B,$Y,$Bd=2592000){global$aa;return
...
query_redirect($F,$Dd,$Qd,$af=true,$fc=true,$lc=false,$gg=""){global$g,$m,$b;if($fc){$Nf=microtime(true);$lc=!$g->query($F);$gg=format_time($Nf);}$Kf="";if($F)$Kf=$b->messageQuery($F,$gg,$lc);if($lc){$m=error().$Kf.script("messagesPrint();");return
false;}if($af)redirect($Dd,$Qd.$Kf);return
true;}function
...
false;}return
true;}function
queries_redirect($Dd,$Qd,$af){list($Ue,$gg)=queries(null);return
query_redirect($Ue,$Dd,$Qd,$af,false,!$af,$gg);}function
...
upload_error($m){$Nd=($m==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($m?'Nepodařilo se nahrát soubor.'.($Nd?" ".sprintf('Maximální povolená velikost souboru je %sB.',$Nd):""):'Soubor neexistuje.');}function
...
lzw_decompress($Ka){$Ib=256;$La=8;$Ya=array();$if=0;$jf=0;for($s=0;$s<strlen($Ka);$s++){$if=($if<<8)+ord($Ka[$s]);$jf+=8;if($jf>=$La){$jf-=$La;$Ya[]=$if>>$jf;$if&=(1<<$jf)-1;$Ib++;if($Ib>>$La)$La++;}}$Hb=range("\0","\xFF");$H="";foreach($Ya
as$s=>$Xa){$Sb=$Hb[$Xa];if(!isset($Sb))$Sb=$ch.$ch[0];$H.=$Sb;if($s)$Hb[]=$ch.$Sb[0];$ch=$Sb;}return$H;}function
...
edit_form($Q,$o,$I,$Hg){global$b,$x,$pg,$m;$Xf=$b->tableName(table_status1($Q,true));page_header(($Hg?'Upravit':'Vložit'),$m,array("select"=>array($Q,$Xf)),$Xf);$b->editRowPrint($Q,$o,$I,$Hg);if($I===false)echo"<p class='error'>".'Žádné řádky.'."\n";echo'<form action="" method="post" enctype="multipart/form-data" id="form">
...
lang($sg,$ce=null){if(is_array($sg)){$Je=($ce==1?0:($ce&&$ce<5?1:2));$sg=$sg[$Je];}$sg=str_replace("%d","%s",$sg);$ce=format_number($ce);return
...
query($F,$Ag=false){$G=$this->pdo->query($F);$this->error="";if(!$G){list(,$this->errno,$this->error)=$this->pdo->errorInfo();if(!$this->error)$this->error='Neznámá chyba.';return
...
connect(){global$b;list(,,$E)=$b->credentials();if($E!="")return'Databáze nepodporuje heslo.';return
new
Min_DB;}function
...
h($g->error);}function
check_sqlite_name($B){global$g;$jc="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($jc)\$~",$B)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$jc));return
false;}return
true;}function
...
false;$g->__construct(":memory:");$g->error='Soubor existuje.';return@rename(DB,$B);}function
...
Min_DB;$vb=$b->credentials();if($g->connect($vb[0],$vb[1],$vb[2])){if(min_version(9,0,$g)){$g->query("SET application_name = 'Adminer'");if(min_version(9.2,0,$g)){$Qf['Řetězce'][]="json";$U["json"]=4294967295;if(min_version(9.4,0,$g)){$Qf['Řetězce'][]="jsonb";$U["jsonb"]=4294967295;}}}return$g;}return$g->error;}function
get_databases(){return
...
set_schema($pf,$h=null){global$g,$U,$Qf;if(!$h)$h=$g;$H=$h->query("SET search_path TO ".idf_escape($pf));foreach(types()as$T){if(!isset($U[$T])){$U[$T]=0;$Qf['Uživatelské typy'][]=$T;}}return$H;}function
...
create_sql($Q,$Da,$Rf){global$g;$H='';$lf=array();$yf=array();$O=table_status($Q);if(is_view($O)){$Rg=view($Q);return
...
view($B){$Rg=views_table("view_name, text");$J=get_rows('SELECT text "select" FROM '.$Rg.' WHERE view_name = '.q($B));return
reset($J);}function
collations(){return
array();}function
information_schema($k){return
...
driver_config(){$U=array();$Qf=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$y=>$X){$U+=$X;$Qf[$y]=array_keys($X);}return
...
MongoClient($Ig,$C);$this->error='Databáze nepodporuje heslo.';}catch(Exception$Pb){}}}catch(Exception$Pb){$this->error=$Pb->getMessage();}}function
query($F){return
false;}function
...
connect(){global$b;$g=new
Min_DB;list($M,$V,$E)=$b->credentials();if($E!=""&&$g->connect($M,$V,""))return'Databáze nepodporuje heslo.';if($g->connect($M,$V,$E))return$g;return$g->error;}function
support($nc){return
...
last_id(){global$g;return$g->last_id;}function
driver_config(){$U=array();$Qf=array();foreach(array('Čísla'=>array("long"=>3,"integer"=>5,"short"=>8,"byte"=>10,"double"=>20,"float"=>66,"half_float"=>12,"scaled_float"=>21),'Datum a čas'=>array("date"=>10),'Řetězce'=>array("string"=>65535,"text"=>65535),'Binární'=>array("binary"=>255),)as$y=>$X){$U+=$X;$Qf[$y]=array_keys($X);}return
...
selectLinks($Wf,$N=""){$a=$Wf["Name"];if($N!==null)echo'<p class="tabs"><a href="'.h(ME.'edit='.urlencode($a).$N).'">'.'Nová položka'."</a>\n";}function
...
as$e=>$X)$_.="&set".urlencode("[".bracket_escape($e)."]")."=".urlencode($I[$X]);echo"<a href='".h($_)."' title='".'Nová položka'."'>+</a> ";}}}function
...
selectVal($X,$_,$n,$te){$H=$X;$_=h($_);if(preg_match('~blob|bytea~',$n["type"])&&!is_utf8($X)){$H=lang(array('%d bajt','%d bajty','%d bajtů'),strlen($te));if(preg_match("~^(GIF|\xFF\xD8\xFF|\x89PNG\x0D\x0A\x1A\x0A)~",$te))$H="<img src='$_' alt='$H'>";}if(like_bool($n)&&$H!="")$H=(preg_match('~^(1|t|true|y|yes|on)$~i',$X)?'ano':'ne');if($_)$H="<a href='$_'".(is_url($_)?target_blank():"").">$H</a>";if(!$_&&!like_bool($n)&&preg_match(number_type(),$n["type"]))$H="<div class='number'>$H</div>";elseif(preg_match('~date~',$n["type"]))$H="<div class='datetime'>$H</div>";return$H;}function
...
as$B=>$Eb){$n=$o[$B];if(preg_match("~enum~",$n["type"])||like_bool($n)){$y=$sd[$B];$s--;echo"<div>".h($Eb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'>:",(like_bool($n)?" <select name='where[$s][val]'>".optionlist(array(""=>"",'ne','ano'),$Z[$y]["val"],true)."</select>":enum_input("checkbox"," name='where[$s][val][]'",$n,(array)$Z[$y]["val"],($n["null"]?0:null))),"</div>\n";unset($f[$B]);}elseif(is_array($C=$this->_foreignKeyOptions($_GET["select"],$B))){if($o[$B]["null"])$C[0]='('.'prázdné'.')';$y=$sd[$B];$s--;echo"<div>".h($Eb)."<input type='hidden' name='where[$s][col]' value='".h($B)."'><input type='hidden' name='where[$s][op]' value='='>: <select name='where[$s][val]'>".optionlist($C,$Z[$y]["val"],true)."</select></div>\n";unset($f[$B]);}}$s=0;foreach($Z
as$X){if(($X["col"]==""||$f[$X["col"]])&&"$X[col]$X[val]"!=""){echo"<div><select name='where[$s][col]'><option value=''>(".'kdekoliv'.")".optionlist($f,$X["col"],true)."</select>",html_select("where[$s][op]",array(-1=>"")+$this->operators,$X["op"]),"<input type='search' name='where[$s][val]' value='".h($X["val"])."'>".script("mixin(qsl('input'), {onkeydown: selectSearchKeydown, onsearch: selectSearchSearch});","")."</div>\n";$s++;}}echo"<div><select name='where[$s][col]'><option value=''>(".'kdekoliv'.")".optionlist($f,null,true)."</select>",script("qsl('select').onchange = selectAddRow;",""),html_select("where[$s][op]",array(-1=>"")+$this->operators),"<input type='search' name='where[$s][val]'></div>",script("mixin(qsl('input'), {onchange: function () { this.parentNode.firstChild.onchange(); }, onsearch: selectSearchSearch});"),"</div></fieldset>\n";}function
selectOrderPrint($pe,$f,$w){$qe=array();foreach($w
as$y=>$v){$pe=array();foreach($v["columns"]as$X)$pe[]=$f[$X];if(count(array_filter($pe,'strlen'))>1&&$y!="PRIMARY")$qe[$y]=implode(", ",$pe);}if($qe){echo'<fieldset><legend>'.'Seřadit'."</legend><div>","<select name='index_order'>".optionlist(array(""=>"")+$qe,($_GET["order"][0]!=""?"":$_GET["index_order"]),true)."</select>","</div></fieldset>\n";}if($_GET["order"])echo"<div style='display: none;'>".hidden_fields(array("order"=>array(1=>reset($_GET["order"])),"desc"=>($_GET["desc"]?array(1=>1):array()),))."</div>\n";}function
...
true;}function
selectEmailPrint($Ub,$f){if($Ub){print_fieldset("email",'E-mail',$_POST["email_append"]);echo"<div>",script("qsl('div').onkeydown = partialArg(bodyKeydown, 'email');"),"<p>".'Odesílatel'.": <input name='email_from' value='".h($_POST?$_POST["email_from"]:$_COOKIE["adminer_email"])."'>\n",'Předmět'.": <input name='email_subject' value='".h($_POST["email_subject"])."'>\n","<p><textarea name='email_message' rows='15' cols='75'>".h($_POST["email_message"].($_POST["email_append"]?'{$'."$_POST[email_addition]}":""))."</textarea>\n","<p>".script("qsl('p').onkeydown = partialArg(bodyKeydown, 'email_append');","").html_select("email_addition",$f,$_POST["email_addition"])."<input type='submit' name='email_append' value='".'Vložit'."'>\n";echo"<p>".'Přílohy'.": <input type='file' name='email_files[]'>".script("qsl('input').onchange = emailFileChange;"),"<p>".(count($Ub)==1?'<input type="hidden" name="email_field" value="'.h(key($Ub)).'">':html_select("email_field",$Ub)),"<input type='submit' name='email' value='".'Odeslat'."'>".confirm(),"</div>\n","</div></fieldset>\n";}}function
...
editFunctions($n){$H=array();if($n["null"]&&preg_match('~blob~',$n["type"]))$H["NULL"]='prázdné';$H[""]=($n["null"]||$n["auto_increment"]||like_bool($n)?"":"*");if(preg_match('~date|time~',$n["type"]))$H["now"]='teď';if(preg_match('~_(md5|sha1)$~i',$n["field"],$A))$H[]=strtolower($A[1]);return$H;}function
editInput($Q,$n,$Aa,$Y){if($n["type"]=="enum")return(isset($_GET["select"])?"<label><input type='radio'$Aa value='-1' checked><i>".'původní'."</i></label> ":"").enum_input("radio",$Aa,$n,($Y||isset($_GET["select"])?$Y:0),($n["null"]?"":null));$C=$this->_foreignKeyOptions($Q,$n["field"],$Y);if($C!==null)return(is_array($C)?"<select$Aa>".optionlist($C,$Y,true)."</select>":"<input value='".h($Y)."'$Aa class='hidden'>"."<input value='".h($C)."' class='jsonly'>"."<div></div>".script("qsl('input').oninput = partial(whisper, '".ME."script=complete&source=".urlencode($Q)."&field=".urlencode($n["field"])."&value=');
qsl('div').onclick = whisperClick;",""));if(like_bool($n))return'<input type="checkbox" value="1"'.(preg_match('~^(1|t|true|y|yes|on)$~i',$Y)?' checked':'')."$Aa>";$Sc="";if(preg_match('~time~',$n["type"]))$Sc='HH:MM:SS';if(preg_match('~date|timestamp~',$n["type"]))$Sc='d.m.[rrrr]'.($Sc?" [$Sc]":"");if($Sc)return"<input value='".h($Y)."'$Aa> ($Sc)";if(preg_match('~_(md5|sha1)$~i',$n["field"]))return"<input type='password' value='".h($Y)."'$Aa>";return'';}function
editHint($Q,$n,$Y){return(preg_match('~\s+(\[.*\])$~',($n["comment"]!=""?$n["comment"]:$n["field"]),$A)?h(" $A[1]"):'');}function
processInput($n,$Y,$r=""){if($r=="now")return"$r()";$H=$Y;if(preg_match('~date|timestamp~',$n["type"])&&preg_match('(^'.str_replace('\$1','(?P<p1>\d*)',preg_replace('~(\\\\\\$([2-6]))~','(?P<p\2>\d{1,2})',preg_quote('$6.$4.$1'))).'(.*))',$Y,$A))$H=($A["p1"]!=""?$A["p1"]:($A["p2"]!=""?($A["p2"]<70?20:19).$A["p2"]:gmdate("Y")))."-$A[p3]$A[p4]-$A[p5]$A[p6]".end($A);$H=($n["type"]=="bit"&&preg_match('~^[0-9]+$~',$Y)?$H:q($H));if($Y==""&&like_bool($n))$H="'0'";elseif($Y==""&&($n["null"]||!preg_match('~char|text~',$n["type"])))$H="NULL";elseif(preg_match('~^(md5|sha1)$~',$r))$H="$r($H)";return
...
';if($Td=="auth"){$tc=true;foreach((array)$_SESSION["pwds"]as$Pg=>$_f){foreach($_f[""]as$V=>$E){if($E!==null){if($tc){echo"<ul id='logins'>",script("mixin(qs('#logins'), {onmouseover: menuOver, onmouseout: menuOut});");$tc=false;}echo"<li><a href='".h(auth_url($Pg,"",$V))."'>".($V!=""?h($V):"<i>".'prázdné'."</i>")."</a>\n";}}}}else{$this->databasesPrint($Td);if($Td!="db"&&$Td!="ns"){$R=table_status('',true);if(!$R)echo"<p class='message'>".'Žádné tabulky.'."\n";else$this->tablesPrint($R);}}}function
databasesPrint($Td){}function
tablesPrint($S){echo"<ul id='tables'>",script("mixin(qs('#tables'), {onmouseover: menuOver, onmouseout: menuOut});");foreach($S
as$I){echo'<li>';$B=$this->tableName($I);if(isset($I["Engine"])&&$B!="")echo"<a href='".h(ME).'select='.urlencode($I["Name"])."'".bold($_GET["select"]==$I["Name"]||$_GET["edit"]==$I["Name"],"select")." title='".'Vypsat data'."'>$B</a>\n";}echo"</ul>\n";}function
...
_foreignKeyOptions($Q,$e,$Y=null){global$g;if(list($ag,$t,$B)=$this->_foreignColumn(column_foreign_keys($Q),$e)){$H=&$this->_values[$ag];if($H===null){$R=table_status($ag);$H=($R["Rows"]>1000?"":array(""=>"")+get_key_vals("SELECT $t, $B FROM ".table($ag)." ORDER BY 2"));}if(!$H&&$Y!==null)return$g->result("SELECT $B FROM ".table($ag)." WHERE $t = ".q($Y));return$H;}}}$b=(function_exists('adminer_object')?adminer_object():new
...
connect($M,$V,$E){if(ini_bool("mysql.allow_local_infile")){$this->error=sprintf('Zakažte %s nebo povolte extenze %s nebo %s.',"'mysql.allow_local_infile'","MySQLi","PDO_MySQL");return
...
as$Q)$Db[table($Q)]=view($Q);$g->select_db($ag);$k=idf_escape(DB);foreach($Db
...
false;}}foreach($Sg
as$Q){$B=($ag==DB?table("copy_$Q"):idf_escape($ag).".".table($Q));$Rg=view($Q);if(($_POST["overwrite"]&&!queries("DROP VIEW IF EXISTS $B"))||!queries("CREATE VIEW $B AS $Rg[select]"))return
false;}return
true;}function
trigger($B){if($B=="")return
...
check_invalid_login(){global$b;$ld=unserialize(@file_get_contents(get_temp_dir()."/adminer.invalid"));$kd=($ld?$ld[$b->bruteForceKey()]:array());$Zd=($kd[1]>29?$kd[0]-time():0);if($Zd>0)auth_error(lang(array('Příliš mnoho pokusů o přihlášení, zkuste to znovu za %d minutu.','Příliš mnoho pokusů o přihlášení, zkuste to znovu za %d minuty.','Příliš mnoho pokusů o přihlášení, zkuste to znovu za %d minut.'),ceil($Zd/60)));}$Ba=$_POST["auth"];if($Ba){session_regenerate_id();$Pg=$Ba["driver"];$M=$Ba["server"];$V=$Ba["username"];$E=(string)$Ba["password"];$k=$Ba["db"];set_password($Pg,$M,$V,$E);$_SESSION["db"][$Pg][$M][$V][$k]=true;if($Ba["permanent"]){$y=base64_encode($Pg)."-".base64_encode($M)."-".base64_encode($V)."-".base64_encode($k);$Pe=$b->permanentLogin(true);$Ge[$y]="$y:".base64_encode($Pe?encrypt_string($E,$Pe):"");cookie("adminer_permanent",implode(" ",$Ge));}if(count($_POST)==1||DRIVER!=$Pg||SERVER!=$M||$_GET["username"]!==$V||DB!=$k)redirect(auth_url($Pg,$M,$V,$k));}elseif($_POST["logout"]&&(!$Oc||verify_token())){foreach(array("pwds","db","dbs","queries")as$y)set_session($y,null);unset_permanent();redirect(substr(preg_replace('~\b(username|db|ns)=[^&]*&~','',ME),0,-1),'Odhlášení proběhlo v pořádku.'.' '.'Díky za použití Admineru, <a href="https://www.adminer.org/cs/donation/">přispějte</a> na vývoj.');}elseif($Ge&&!$_SESSION["pwds"]){session_regenerate_id();$Pe=$b->permanentLogin();foreach($Ge
...
as$y=>$X){list($Pg,$M,$V,$k)=array_map('base64_decode',explode("-",$y));if($Pg==DRIVER&&$M==SERVER&&$V==$_GET["username"]&&$k==DB)unset($Ge[$y]);}cookie("adminer_permanent",implode(" ",$Ge));}function
auth_error($m){global$b,$Oc;$Af=session_name();if(isset($_GET["username"])){header("HTTP/1.1 403 Forbidden");if(($_COOKIE[$Af]||$_GET[$Af])&&!$Oc)$m='Session vypršela, přihlašte se prosím znovu.';else{restart_session();add_invalid_login();$E=get_password();if($E!==null){if($E===false)$m.=($m?'<br>':'').sprintf('Platnost hlavního hesla vypršela. <a href="https://www.adminer.org/cs/extension/"%s>Implementujte</a> metodu %s, aby platilo stále.',target_blank(),'<code>permanentLogin()</code>');set_password(DRIVER,SERVER,$_GET["username"],null);}unset_permanent();}}if(!$_COOKIE[$Af]&&$_GET[$Af]&&ini_bool("session.use_only_cookies"))$m='Session proměnné musí být povolené.';$_e=session_get_cookie_params();cookie("adminer_key",($_COOKIE["adminer_key"]?$_COOKIE["adminer_key"]:rand_string()),$_e["lifetime"]);page_header('Přihlásit se',$m,null);echo"<form action='' method='post'>\n","<div>";if(hidden_fields($_POST,array("auth")))echo"<p class='message'>".'Akce bude provedena po úspěšném přihlášení se stejnými přihlašovacími údaji.'."\n";echo"</div>\n";$b->loginForm();echo"</form>\n";page_footer("auth");exit;}if(isset($_GET["username"])&&!class_exists("Min_DB")){unset($_SESSION["pwds"][DRIVER]);unset_permanent();page_header('Žádné rozšíření',sprintf('Není dostupné žádné z podporovaných PHP rozšíření (%s).',implode(", ",$Ke)),false);page_footer("auth");exit;}stop_session(true);if(isset($_GET["username"])&&is_string(get_password())){list($Uc,$Ie)=explode(":",SERVER,2);if(preg_match('~^\s*([-+]?\d+)~',$Ie,$A)&&($A[1]<1024||$A[1]>65535))auth_error('Připojování k privilegovaným portům není povoleno.');check_invalid_login();$g=connect();$l=new
Min_Driver($g);}$Ed=null;if(!is_object($g)||($Ed=$b->login($_GET["username"],get_password()))!==true){$m=(is_string($g)?h($g):(is_string($Ed)?$Ed:'Neplatné přihlašovací údaje.'));auth_error($m.(preg_match('~^ | $~',get_password())?'<br>'.'Problém může být, že je v zadaném hesle mezera.':''));}if($_POST["logout"]&&$Oc&&!verify_token()){page_header('Odhlásit','Neplatný token CSRF. Odešlete formulář znovu.');page_footer("db");exit;}if($Ba&&$_POST["token"])$_POST["token"]=$pg;$m='';if($_POST){if(!verify_token()){$hd="max_input_vars";$Od=ini_get($hd);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$Od||$X<$Od)){$hd=$y;$Od=$X;}}}$m=(!$_POST["token"]&&$Od?sprintf('Byl překročen maximální povolený počet polí. Zvyšte prosím %s.',"'$hd'"):'Neplatný token CSRF. Odešlete formulář znovu.'.' '.'Pokud jste tento požadavek neposlali z Adminera, tak tuto stránku zavřete.');}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$m=sprintf('Příliš velká POST data. Zmenšete data nebo zvyšte hodnotu konfigurační direktivy %s.',"'post_max_size'");if(isset($_GET["sql"]))$m.=' '.'Velký SQL soubor můžete nahrát pomocí FTP a importovat ho ze serveru.';}function
...
as$y=>$X){if(in_array(idf_escape($y),$K))unset($Fg[$y]);}break;}}if($fe&&!$Me){$Me=$Fg=array($fe=>0);$w[]=array("type"=>"PRIMARY","columns"=>array($fe));}if($_POST&&!$m){$Zg=$Z;if(!$_POST["all"]&&is_array($_POST["check"])){$Ua=array();foreach($_POST["check"]as$Ra)$Ua[]=where_check($Ra,$o);$Zg[]="((".implode(") OR (",$Ua)."))";}$Zg=($Zg?"\nWHERE ".implode(" AND ",$Zg):"");if($_POST["export"]){cookie("adminer_import","output=".urlencode($_POST["output"])."&format=".urlencode($_POST["format"]));dump_headers($a);$b->dumpTable($a,"");$Fc=($K?implode(", ",$K):"*").convert_fields($f,$o,$K)."\nFROM ".table($a);$Jc=($Hc&&$nd?"\nGROUP BY ".implode(", ",$Hc):"").($pe?"\nORDER BY ".implode(", ",$pe):"");if(!is_array($_POST["check"])||$Me)$F="SELECT $Fc$Zg$Jc";else{$Bg=array();foreach($_POST["check"]as$X)$Bg[]="(SELECT".limit($Fc,"\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$o).$Jc,1).")";$F=implode(" UNION ALL ",$Bg);}$b->dumpData($a,"table",$F);exit;}if(!$b->selectEmailProcess($Z,$Bc)){if($_POST["save"]||$_POST["delete"]){$G=true;$ta=0;$N=array();if(!$_POST["delete"]){foreach($f
as$B=>$X){$X=process_input($o[$B]);if($X!==null&&($_POST["clone"]||$X!==false))$N[idf_escape($B)]=($X!==false?$X:idf_escape($B));}}if($_POST["delete"]||$N){if($_POST["clone"])$F="INTO ".table($a)." (".implode(", ",array_keys($N)).")\nSELECT ".implode(", ",$N)."\nFROM ".table($a);if($_POST["all"]||($Me&&is_array($_POST["check"]))||$nd){$G=($_POST["delete"]?$l->delete($a,$Zg):($_POST["clone"]?queries("INSERT $F$Zg"):$l->update($a,$N,$Zg)));$ta=$g->affected_rows;}else{foreach((array)$_POST["check"]as$X){$Vg="\nWHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($X,$o);$G=($_POST["delete"]?$l->delete($a,$Vg,1):($_POST["clone"]?queries("INSERT".limit1($a,$F,$Vg)):$l->update($a,$N,$Vg,1)));if(!$G)break;$ta+=$g->affected_rows;}}}$Qd=lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ta);if($_POST["clone"]&&$G&&$ta==1){$yd=last_id();if($yd)$Qd=sprintf('Položka%s byla vložena.'," $yd");}queries_redirect(remove_from_uri($_POST["all"]&&$_POST["delete"]?"page":""),$Qd,$G);if(!$_POST["delete"]){edit_form($a,$o,(array)$_POST["fields"],!$_POST["clone"]);page_footer();exit;}}elseif(!$_POST["import"]){if(!$_POST["val"])$m='Ctrl+klikněte na políčko, které chcete změnit.';else{$G=true;$ta=0;foreach($_POST["val"]as$Dg=>$I){$N=array();foreach($I
as$y=>$X){$y=bracket_escape($y,1);$N[idf_escape($y)]=(preg_match('~char|text~',$o[$y]["type"])||$X!=""?$b->processInput($o[$y],$X):"NULL");}$G=$l->update($a,$N," WHERE ".($Z?implode(" AND ",$Z)." AND ":"").where_check($Dg,$o),!$nd&&!$Me," ");if(!$G)break;$ta+=$g->affected_rows;}queries_redirect(remove_from_uri(),lang(array('Byl ovlivněn %d záznam.','Byly ovlivněny %d záznamy.','Bylo ovlivněno %d záznamů.'),$ta),$G);}}elseif(!is_string($qc=get_file("csv_file",true)))$m=upload_error($qc);elseif(!preg_match('~~u',$qc))$m='Soubor musí být v kódování UTF-8.';else{cookie("adminer_import","output=".urlencode($sa["output"])."&format=".urlencode($_POST["separator"]));$G=true;$cb=array_keys($o);preg_match_all('~(?>"[^"]*"|[^"\r\n]+)+~',$qc,$Kd);$ta=count($Kd[0]);$l->begin();$L=($_POST["separator"]=="csv"?",":($_POST["separator"]=="tsv"?"\t":";"));$J=array();foreach($Kd[0]as$y=>$X){preg_match_all("~((?>\"[^\"]*\")+|[^$L]*)$L~",$X.$L,$Ld);if(!$y&&!array_diff($Ld[1],$cb)){$cb=$Ld[1];$ta--;}else{$N=array();foreach($Ld[1]as$s=>$Za)$N[idf_escape($cb[$s])]=($Za==""&&$o[$cb[$s]]["null"]?"NULL":q(str_replace('""','"',preg_replace('~^"|"$~','',$Za))));$J[]=$N;}}$G=(!$J||$l->insertUpdate($a,$J,$Me));if($G)$G=$l->commit();queries_redirect(remove_from_uri("page"),lang(array('Byl importován %d záznam.','Byly importovány %d záznamy.','Bylo importováno %d záznamů.'),$ta),$G);$l->rollback();}}}$Xf=$b->tableName($R);if(is_ajax()){page_headers();ob_start();}else
page_header('Vypsat'.": $Xf",$m);$N=null;if(isset($mf["insert"])||!support("table")){$N="";foreach((array)$_GET["where"]as$X){if($Bc[$X["col"]]&&count($Bc[$X["col"]])==1&&($X["op"]=="="||(!$X["op"]&&!preg_match('~[_%]~',$X["val"]))))$N.="&set".urlencode("[".bracket_escape($X["col"])."]")."=".urlencode($X["val"]);}}$b->selectLinks($R,$N);if(!$f&&support("table"))echo"<p class='error'>".'Nepodařilo se vypsat tabulku'.($o?".":": ".error())."\n";else{echo"<form action='' id='form'>\n","<div style='display: none;'>";hidden_fields_get();echo(DB!=""?'<input type="hidden" name="db" value="'.h(DB).'">'.(isset($_GET["ns"])?'<input type="hidden" name="ns" value="'.h($_GET["ns"]).'">':""):"");echo'<input type="hidden" name="select" value="'.h($a).'">',"</div>\n";$b->selectColumnsPrint($K,$f);$b->selectSearchPrint($Z,$f,$w);$b->selectOrderPrint($pe,$f,$w);$b->selectLimitPrint($z);$b->selectLengthPrint($eg);$b->selectActionPrint($w);echo"</form>\n";$D=$_GET["page"];if($D=="last"){$Dc=$g->result(count_rows($a,$Z,$nd,$Hc));$D=floor(max(0,$Dc-1)/$z);}$sf=$K;$Ic=$Hc;if(!$sf){$sf[]="*";$qb=convert_fields($f,$o,$K);if($qb)$sf[]=substr($qb,2);}foreach($K
as$y=>$X){$n=$o[idf_unescape($X)];if($n&&($ya=convert_field($n)))$sf[$y]="$ya AS $X";}if(!$nd&&$Fg){foreach($Fg
as$y=>$X){$sf[]=idf_escape($y);if($Ic)$Ic[]=idf_escape($y);}}$G=$l->select($a,$sf,$Z,$Ic,$pe,$z,$D,true);if(!$G)echo"<p class='error'>".error()."\n";else{if($x=="mssql"&&$D)$G->seek($z*$D);$Vb=array();echo"<form action='' method='post' enctype='multipart/form-data'>\n";$J=array();while($I=$G->fetch_assoc()){if($D&&$x=="oracle")unset($I["RNUM"]);$J[]=$I;}if($_GET["page"]!="last"&&$z!=""&&$Hc&&$nd&&$x=="sql")$Dc=$g->result(" SELECT FOUND_ROWS()");if(!$J)echo"<p class='message'>".'Žádné řádky.'."\n";else{$Ia=$b->backwardKeys($a,$Xf);echo"<div class='scrollable'>","<table id='table' cellspacing='0' class='nowrap checkable'>",script("mixin(qs('#table'), {onclick: tableClick, ondblclick: partialArg(tableClick, true), onkeydown: editingKeydown});"),"<thead><tr>".(!$Hc&&$K?"":"<td><input type='checkbox' id='all-page' class='jsonly'>".script("qs('#all-page').onclick = partial(formCheck, /check/);","")." <a href='".h($_GET["modify"]?remove_from_uri("modify"):$_SERVER["REQUEST_URI"]."&modify=1")."'>".'Změnit'."</a>");$Xd=array();$Gc=array();reset($K);$Ye=1;foreach($J[0]as$y=>$X){if(!isset($Fg[$y])){$X=$_GET["columns"][key($K)];$n=$o[$K?($X?$X["col"]:current($K)):$y];$B=($n?$b->fieldName($n,$Ye):($X["fun"]?"*":$y));if($B!=""){$Ye++;$Xd[$y]=$B;$e=idf_escape($y);$Vc=remove_from_uri('(order|desc)[^=]*|page').'&order%5B0%5D='.urlencode($y);$Eb="&desc%5B0%5D=1";echo"<th id='th[".h(bracket_escape($y))."]'>".script("mixin(qsl('th'), {onmouseover: partial(columnMouse), onmouseout: partial(columnMouse, ' hidden')});",""),'<a href="'.h($Vc.($pe[0]==$e||$pe[0]==$y||(!$pe&&$nd&&$Hc[0]==$e)?$Eb:'')).'">';echo
apply_sql_function($X["fun"],$B)."</a>";echo"<span class='column hidden'>","<a href='".h($Vc.$Eb)."' title='".'sestupně'."' class='text'> ↓</a>";if(!$X["fun"]){echo'<a href="#fieldset-search" title="'.'Vyhledat'.'" class="text jsonly"> =</a>',script("qsl('a').onclick = partial(selectSearch, '".js_escape($y)."');");}echo"</span>";}$Gc[$y]=$X["fun"];next($K);}}$Ad=array();if($_GET["modify"]){foreach($J
as$I){foreach($I
as$y=>$X)$Ad[$y]=max($Ad[$y],min(40,strlen(utf8_decode($X))));}}echo($Ia?"<th>".'Vztahy':"")."</thead>\n";if(is_ajax()){if($z%2==1&&$D%2==1)odd();ob_end_clean();}foreach($b->rowDescriptions($J,$Bc)as$Wd=>$I){$Cg=unique_array($J[$Wd],$w);if(!$Cg){$Cg=array();foreach($J[$Wd]as$y=>$X){if(!preg_match('~^(COUNT\((\*|(DISTINCT )?`(?:[^`]|``)+`)\)|(AVG|GROUP_CONCAT|MAX|MIN|SUM)\(`(?:[^`]|``)+`\))$~',$y))$Cg[$y]=$X;}}$Dg="";foreach($Cg
...
as$qd=>$W)$_.=where_link($s++,$qd,$W);}$X=select_value($X,$_,$n,$eg);$t=h("val[$Dg][".bracket_escape($y)."]");$Y=$_POST["val"][$Dg][bracket_escape($y)];$Rb=!is_array($I[$y])&&is_utf8($X)&&$J[$Wd][$y]==$I[$y]&&!$Gc[$y];$dg=preg_match('~text|lob~',$n["type"]);echo"<td id='$t'";if(($_GET["modify"]&&$Rb)||$Y!==null){$Lc=h($Y!==null?$Y:$I[$y]);echo">".($dg?"<textarea name='$t' cols='30' rows='".(substr_count($I[$y],"\n")+1)."'>$Lc</textarea>":"<input name='$t' value='$Lc' size='$Ad[$y]'>");}else{$Fd=strpos($X,"<i>…</i>");echo" data-text='".($Fd?2:($dg?1:0))."'".($Rb?"":" data-warning='".h('Ke změně této hodnoty použijte odkaz upravit.')."'").">$X</td>";}}}if($Ia)echo"<td>";$b->backwardKeysPrint($Ia,$J[$Wd]);echo"</tr>\n";}if(is_ajax())exit;echo"</table>\n","</div>\n";}if(!is_ajax()){if($J||$D){$ec=true;if($_GET["page"]!="last"){if($z==""||(count($J)<$z&&($J||!$D)))$Dc=($D?$D*$z:0)+count($J);elseif($x!="sql"||!$nd){$Dc=($nd?false:found_rows($R,$Z));if($Dc<max(1e4,2*($D+1)*$z))$Dc=reset(slow_query(count_rows($a,$Z,$nd,$Hc)));else$ec=false;}}$ye=($z!=""&&($Dc===false||$Dc>$z||$D));if($ye){echo(($Dc===false?count($J)+1:$Dc-$D*$z)>$z?'<p><a href="'.h(remove_from_uri("page")."&page=".($D+1)).'" class="loadmore">'.'Nahrát další data'.'</a>'.script("qsl('a').onclick = partial(selectLoadMore, ".(+$z).", '".'Nahrává se'."…');",""):''),"\n";}}echo"<div class='footer'><div>\n";if($J||$D){if($ye){$Md=($Dc===false?$D+(count($J)>=$z?2:1):floor(($Dc-1)/$z));echo"<fieldset>";if($x!="simpledb"){echo"<legend><a href='".h(remove_from_uri("page"))."'>".'Stránka'."</a></legend>",script("qsl('a').onclick = function () { pageClick(this.href, +prompt('".'Stránka'."', '".($D+1)."')); return false; };"),pagination(0,$D).($D>5?" …":"");for($s=max(1,$D-4);$s<min($Md,$D+5);$s++)echo
pagination($s,$D);if($Md>0){echo($D+5<$Md?" …":""),($ec&&$Dc!==false?pagination($Md,$D):" <a href='".h(remove_from_uri("page")."&page=last")."' title='~$Md'>".'poslední'."</a>");}}else{echo"<legend>".'Stránka'."</legend>",pagination(0,$D).($D>1?" …":""),($D?pagination($D,$D):""),($Md>$D?pagination($D+1,$D).($Md>$D+1?" …":""):"");}echo"</fieldset>\n";}echo"<fieldset>","<legend>".'Celý výsledek'."</legend>";$Jb=($ec?"":"~ ").$Dc;echo
checkbox("all",1,0,($Dc!==false?($ec?"":"~ ").lang(array('%d řádek','%d řádky','%d řádků'),$Dc):""),"var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Jb' : checked); selectCount('selected2', this.checked || !checked ? '$Jb' : checked);")."\n","</fieldset>\n";if($b->selectCommandPrint()){echo'<fieldset',($_GET["modify"]?'':' class="jsonly"'),'><legend>Změnit</legend><div>
